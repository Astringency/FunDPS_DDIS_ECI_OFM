...
for pde in ['poisson', 'helmholtz', 'darcy', 'nsnonbounded', 'burger']:
    ...
    for batch in [1, 4, 8]:
        ...
        for index in range(4):
            ...
            assert np.isfinite(delta) and delta < 1e-3, (a.prior, pde, batch, index, delta)
...
# Full schedule against already verified sequential outputs; no repeated formal evaluation.
pde = 'poisson'
dest = root / pde / 'full_b8'
dest.mkdir(exist_ok=True)
assets = OUT / 'data/shared_prior_assets'
ckpt = (OUT / 'jobs/flow_poisson/early_stopping/best_checkpoint').resolve() if a.prior == 'ofm' else assets / 'weights/poisson.pth'
cmd = [sys.executable, '-u', AD / 'evaluate_shared_prior.py', '--prior', a.prior, '--method', 'eci',
       '--pde', pde, '--task', 'inverse', '--split', 'id', '--count', '8', '--checkpoint', ckpt,
       '--assets', assets, '--source', OUT / 'data/compact/poisson', '--fm4pde', BASE / 'official/FM4PDE-cbe627c',
       '--ofm', BASE / 'official/OFM', '--eci', BASE / 'official/ECI', '--output', dest, '--eci-batch-size', '8', '--profile']
with (dest / 'sampling.log').open('w') as log:
    subprocess.run(list(map(str, cmd)), stdout=log, stderr=subprocess.STDOUT, check=True)
full = []
for index in range(8):
    ref = OUT / 'evaluation_v2' / a.prior / 'eci/poisson/inverse/id/shard_000' / f'prediction_{index:03d}.npy'
    baseline = np.load(ref)
    actual = np.load(dest / f'prediction_{index:03d}.npy')
    delta = float(np.linalg.norm(actual - baseline) / max(np.linalg.norm(baseline), 1e-30))
    full.append({'sample_id': index, 'relative_difference': delta, 'reference': str(ref)})
    assert np.isfinite(delta) and delta < 1e-3, full[-1]
with (dest / 'verification.log').open('w') as log:
    subprocess.run([sys.executable, str(AD / 'validate_shared_prior.py'), '--output', str(dest)], stdout=log, stderr=subprocess.STDOUT, check=True)
(root / 'full_schedule_agreement.json').write_text(json.dumps(full, indent=2))
(root / 'passed').write_text('All five PDE batch checks and full-schedule Poisson agreement passed.\n')
print('PASSED', a.prior, flush=True)
