...
- Eight `ddis_eval_{method}_{pde}_20260920` tmux controllers were queued at server time 01:25. `adapters/formal_evaluate.sh` waits for completed early-stopping checkpoint selection (and DDIS's surrogate), profiles one case with trained weights, then calls official samplers for ID/Smooth/Rough 100 cases each. It prioritizes remaining training queues, checks free GPU/host memory and holds an experiment GPU lock. Per-split logs are in `jobs/evaluate_{method}_{pde}/`.
- `adapters/validate_evaluation.py` verifies source hashes, exact IDs, common masks, all prediction shapes/finiteness and independently recalculated physical relative L2 errors. It reports uncapped errors and failures separately; it never substitutes a successful-case mean for an all-case mean. Each formal split must have `verified_summary.json` covering 100 cases before a method/PDE controller writes `evaluation_completed`.
